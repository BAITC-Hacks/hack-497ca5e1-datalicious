...
Реализованы данные, контракты и детерминированный доменный движок.
Пользовательская форма, графики и AI-анализ ещё не реализованы. Recharts
установлен для графиков; `POST /api/analyze` пока возвращает 501.
...
TypeScript не проверяет JSON по сети: `validateScenario` использует Zod для
проверки формы, неизвестных ID, лишних полей и бизнес-ограничений.
...
версию датасета, вклады решений и сработавшие синергии. Снимки включают оценки
районов, показатели и критические значения. `contributions` — реализованные
эффекты до clip, а не аддитивные вклады в итоговый Score: min, clip и штрафы
нелинейны. Дельты районов уже учитывают clip.
...
Vitest проверяет целостность данных, `tests/domain`, страницу через RTL
и текущий HTTP-контракт.
Один Playwright smoke запускается на production-сборке: страница, отсутствие
pageerror, API 501. После реализации UI и AI заменить проверку заглушки на
выбор пяти мер и анализ с mock AI, сохраняя один smoke-тест. AI-тесты описаны
в PLAN.
