...
import { baselineSnapshot } from "../simulator/baseline";
import { SimulationResults } from "../simulator/simulation-results";
import {
  readSavedDraft,
  selectionAdapter,
  selectionCost,
...
export function CityStudio() {
  ...
  function launch() {
    ...
    // The animation is presentation only. Calculate with the public engine first.
    try {
      const evaluated = simulationEngine.evaluate(scenario);
      if (!evaluated.ok) {
        setNotice(evaluated.issues.map((issue) => issue.message).join(" "));
        return;
      }
      setResult(evaluated.result);
    } catch {
      setNotice("Не удалось рассчитать сценарий. Объяснение не запрашивалось; попробуйте ещё раз.");
      return;
    }
    setTarget(null);
    setPlanOpen(false);
  ...
  return (
    <div className={`city-studio ${target ? "has-panel" : ""} phase-${phase}`}>
      <div className="studio-content" inert={phase === "complete"}>
            ...
            <small>
              {validation.valid
                ...
                : scenario.decisions.length < dataset.rules.requiredDecisions
                  ? `Выберите ещё ${dataset.rules.requiredDecisions - scenario.decisions.length} меропр.`
        ...
        {rules && (
          ...
          >
            ...
            <p>
              ...
              <strong>{formatNumber(baselineSnapshot.score, 2)}</strong>. Данные
      ...
      {phase === "complete" && result && (
        <SimulationResults
          ...
          onReset={() => {
            change({ decisions: [] });
            setTarget(null);
            requestAnimationFrame(() => document.getElementById("district-picker")?.focus());
          }}
        />
      )}
    </div>
